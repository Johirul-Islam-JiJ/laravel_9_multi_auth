...
@section('breadcrumb')
    ...
    <div class="breadcrumb-wrapper">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('role.index') }}">{{__('Role')}}</a>
            </li>
                            ...
                            <div class="row">
                                <div class="col-12 mb-2">
                                    ...
                                    <input type="text" name="name"
                                           ...
                                           placeholder="Role name" value="{{ old('name') }}" required="" autofocus>
                                    @error('name')
                                            ...
                                            <tr class="row">
                                                ...
                                                <td width="40%" style="vertical-align:top" class="role_{{$loop->iteration}}_management_td">
                                                    @foreach($modules as $permission)
                                                        <div class="custom-control custom-checkbox mb-1">
                                                            <input type="checkbox" name="permissions[]" class="custom-control-input {{$index}}"
                                                                   id="exampleCheckbox{{$permission->id}}"
                                                                   value="{{$permission->id}}">
                                                            <label class="custom-control-label" for="exampleCheckbox{{$permission->id}}">{{ucfirst(str_replace(['.', '-'], ' ', $permission->name))}}</label>
                                                        </div>
                                                    @endforeach
                                                </td>
                                            </tr>
