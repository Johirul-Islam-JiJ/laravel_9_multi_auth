...
@section('breadcrumb')
    ...
    <div class="breadcrumb-wrapper">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('seller.user') }}">{{__('User')}}</a>
            </li>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('seller.user.update', encrypt($seller->id)) }}" method="POST">
                            @csrf
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        <select name="roles[]" id="category" class="form-control select2" multiple>
                                            ...
                                            @foreach($roles as $role)
                                                <option
                                                    value="{{$role->id}}" {{ in_array($role->name,$seller->getRoleNames()->toArray()) ? 'selected':'' }}>{{$role->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-4 col-12 mb-2">
                                    ...
                                    <input type="text" name="name" class="form-control @error('name')is-invalid @enderror" id="name" placeholder="User name" value="{{ $seller->name }}"  autocomplete="off">
                                    @error('name')
                                ...
                                <div class="col-sm-4 col-12 mb-2">
                                    ...
                                    <input type="text" name="phone_number"
                                           ...
                                           id="phone_number"
                                           placeholder="Enter Phone Number" value="{{ $seller->phone_number }}"
                                ...
                                <div class="col-sm-4 col-12 mb-2">
                                    ...
                                    <input type="email" name="email"
                                           ...
                                           placeholder="Enter email" value=" {{ $seller->email }}"
                                           required="">
                                    @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
